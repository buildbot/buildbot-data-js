          ...
          return indexedDBService.db[type].put(message).then((function(_this) {
            return function() {
              ...
              for (path in _this.trackedPaths) {
                if (RegExp("^" + (path.replace(/\*/g, '(\\w+|\\d+)')) + "$").test(key) || (type === 'logs' && /logs/g.test(key))) {
  ...
  Wrapper = (function() {
    function Wrapper($log, dataService, dataUtilsService, tabexService, SPECIFICATION) {
      ...
      return WrapperInstance = (function() {
        ...
        WrapperInstance.prototype._findMatch = function(root, path) {
          var specification;
          specification = SPECIFICATION[root];
          return specification.paths.filter(function(p) {
            var replaced;
            replaced = p.replace(/\w+\:\w+/g, '(\\*|\\w+|\\d+)');
            return RegExp("^" + replaced + "$").test(path);
          }).pop();
        };

        WrapperInstance.prototype.get = function() {
          var args, base, e, fieldName, fieldType, i, id, j, last, match, options, parameter, path, pathString, ref, ref1, ref2, root;
          args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
          ...
          match = this._findMatch(root, pathString);
          if (match == null) {
        ...
        WrapperInstance.prototype.control = function(method, params) {
          var e, fieldName, fieldType, i, id, match, parameter, path, pathString, ref, ref1, ref2, root;
          ref = this._endpoint.split('/'), root = ref[0], id = ref[1], path = 3 <= ref.length ? slice.call(ref, 2) : [];
          pathString = path.concat('*').join('/');
          match = this._findMatch(root, pathString);
          if (match == null) {
            parameter = this.getId() || this.getIdentifier();
          } else {
            ref1 = match.split('/');
          ...
          return dataService.control(this._endpoint + "/" + parameter, method, params);
        };
